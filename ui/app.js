...
async function exportTasks() {
  ...
  for (const t of taskCards) {
    for (const macroName of [t.macro, t.act4_macro]) {
      if (macroName && !(macroName in templates)) {
        try { templates[macroName] = await pywebview.api.load_template(macroName); } catch (e) {}
      }
    }
  }
