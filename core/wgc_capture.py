...
WHY THIS EXISTS (fork addition): on hardware-accelerated / flip-model Roblox,
the stock capture paths (mss BitBlt screen grab, PrintWindow window capture)
come back BLACK once the game is the foreground window, so every image search
fails even though the game is clearly on screen. WGC reads the window's
composed frames straight from DWM and returns real pixels regardless of
occlusion or flip-model presentation -- it's what OBS's "Windows Graphics
Capture" and Xbox Game Bar use.

Runs ONE background capture session on the top-level window titled "Roblox"
(requires cutout mode so Roblox stays top-level and findable by title) and
caches the latest frame; vision.capture_game_gray reads it on demand. Falls
back silently (returns None) if WGC or the window isn't available, so the
original capture paths still run.
"""
import threading
import time
