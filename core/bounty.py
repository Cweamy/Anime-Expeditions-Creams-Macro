...
def _extract_wave_targets(reads: list, *, allow_clear_number: bool = False) -> list:
    """Extract wave targets from OCR without trusting unrelated numbers.

    Windows OCR can return ``CleuWave``, ``Clear.vave`` or ``Wave`` for the
    same label. Keep the number tied to a fuzzy wave token, and only allow a
    bare ``Clear 30`` fallback for a tight card crop where OCR dropped the
    word ``Wave``. This prevents a bounty/card number or progress value from
    becoming the wave target.
    """
    targets = []
    for raw in reads or []:
        raw = str(raw or "")
        ...
        for match in words:
            ...
            # A clipped zero has appeared as C after the leading 6. Restrict
            # this repair to the exact two-character final-slot shape so a
            # trailing OCR artifact such as ``60C`` is not accepted.
            if raw_token[-1:] in ("C", "c"):
                if len(raw_token) != 2:
                    continue
                raw_token = raw_token[:-1] + "0"
            token = raw_token.translate(str.maketrans({
                "O": "0", "o": "0", "S": "5", "s": "5",
                "Z": "2", "z": "2", "I": "1", "i": "1",
                "L": "1", "l": "1",
            }))
...
def _wave_read_quality(text: str) -> int:
    """Score OCR evidence, not an engine confidence value."""
    text = str(text or "")
    ...
    score = (2 if has_wave else 0) + (1 if has_clear else 0)
    if re.search(r"\bof\b", text, re.IGNORECASE):
        score += 1
    return score
...
def _choose_wave_target(read_groups: list) -> int | None:
    ...
    Card-normalized threshold reads are the strongest evidence when at least
    two agree. A single normalized raw read is still preferred over repeated
    unnormalized reads because the latter are where the 20/30 confusion was
    observed. Context text remains the final fallback.
    """
    grouped = {}
    ...
    # Keep the older three-field group API usable for focused unit tests and
    # let the caller's explicit priority decide among remaining evidence.
    scores = Counter()
    evidence = {}
    ...
    for link in links:
        ...
        if ocr_lines is None:
            ...
            if local.size:
                ...
                if link["kind"] == "infinite" and needs_wave_fallback:
                    ...
                    if matched_card is not None:
                        ...
                        # If the card outline is temporarily unavailable,
                        # still try one local contrast pass. It is cheaper
                        # than widening board OCR and remains card-relative.
