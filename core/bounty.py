...
_CARD_RARITIES = ("common", "uncommon", "rare", "epic", "legendary", "mythic")
...
def _classify_card_rarity_texts(texts) -> str | None:
    """Classify rarity from text already scoped to one card title."""
    best = []
    for raw in texts or []:
        text = str(raw or "")
        words = re.findall(r"[A-Za-z]{3,14}", text)
        bounty_score = max(
...
def read_card_rarity(frame_bgr: np.ndarray, card, ocr_lines=None) -> str | None:
    ...
    The full-frame OCR result is used first for speed. A local title retry is
    only paid when that card-local line did not classify.
    """
...
def detect_reroll_buttons(frame_bgr: np.ndarray, cards=None) -> list:
    """Locate active gold reroll buttons inside detected card footers.

    The gray X beside the button is intentionally outside the accepted gold
    component. Bounds are derived from each live card rectangle so the click
    follows UI scale and window position.
    """
    if (frame_bgr is None or not hasattr(frame_bgr, "shape")
    ...
    for item in cards:
        ...
        # The reroll is the rightmost footer control. The left half contains
        # the pin and gray X, while the gold cost icon sits above it.
