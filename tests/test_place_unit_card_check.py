...
# ---------------------------------------------------------------------------
# Two units never land on one tile
# ---------------------------------------------------------------------------

def _scan_runner(taken=None):
    r = _Runner()
    r._placed_points_this_match = list(taken or [])
    return r


def _white_at(monkeypatch, points, size=9):
    """Capture stand-in painting a white blob at each WINDOW coordinate."""
    import numpy as np

    def capture(_hwnd, region):
        x, y, w, h = region
        patch = np.zeros((h, w, 3), np.uint8)
        for px, py in points:
            cx, cy = px - x, py - y
            x0, x1 = max(0, cx - size // 2), min(w, cx + size // 2 + 1)
            y0, y1 = max(0, cy - size // 2), min(h, cy + size // 2 + 1)
            if x0 < x1 and y0 < y1:
                patch[y0:y1, x0:x1] = (255, 255, 255)
        return patch

    monkeypatch.setattr("core.runner_blocks.vision.capture_window_region_bgr", capture)
    monkeypatch.setattr("core.ocr.capture_region",
                        lambda l, t, w, h: capture(0, (l, t, w, h)))


def test_a_free_tile_is_accepted_normally(monkeypatch):
    _white_at(monkeypatch, [(576, 378)])
    assert _scan_runner()._scan_place_search_box(1, 0, 0, 576, 378) == (0, 0)


def test_a_tile_already_holding_a_unit_is_refused(monkeypatch):
    """The East Town failure: Cell 2 aimed 16px from Cell 1, same tile. The
    scan must keep looking rather than settle there, so the wiggle and the
    outward spiral get their chance."""
    _white_at(monkeypatch, [(576, 378)])
    assert _scan_runner(taken=[(576, 378)])._scan_place_search_box(1, 0, 0, 576, 378) is None


def test_it_settles_on_the_nearest_free_tile_instead(monkeypatch):
    """With the occupied tile off to one side, the scan should skip thewhite
    candidates hugging it and take the far one -- the nearest-pixel rule
    would otherwise pick the closer, unusable side."""
    _white_at(monkeypatch, [(562, 378), (590, 378)])

    got = _scan_runner(taken=[(550, 378)])._scan_place_search_box(1, 0, 0, 576, 378)

    assert got is not None, "refused the occupied side AND the free one"
    assert got[0] > 0, f"settled on the blob hugging the occupied tile: {got}"


def test_everything_in_the_box_being_occupied_reports_nothing(monkeypatch):
    """Nothing within +/-19px can be PLACE_MIN_SEPARATION from a unit sitting
    at the middle of the box, so this scan has no answer -- and saying so is
    what escalates to the outward spiral instead of clicking a dead tile."""
    _white_at(monkeypatch, [(576, 378)])
    assert _scan_runner(taken=[(576, 378)])._scan_place_search_box(1, 0, 0, 576, 378) is None


def test_separation_is_per_match_not_forever(monkeypatch):
    """_placed_unit_positions is built once in __init__ and never cleared, so
    the occupied list has to be its own per-match thing -- otherwise last
    match's tiles stay blocked in this one."""
    _white_at(monkeypatch, [(576, 378)])
    assert _scan_runner(taken=[])._scan_place_search_box(1, 0, 0, 576, 378) == (0, 0)


def test_a_placement_records_its_tile_as_occupied(placing, monkeypatch):
    monkeypatch.setattr(runner_blocks.vision, "read_unit_card",
                        lambda h, s: {"in_hand": True, "affordable": True, "price_px": 660, "hue": 45})
    placing._unplaced_units = {}
    placing._placed_points_this_match = []

    _place(placing)

    assert placing._placed_points_this_match == [(500, 400)]
