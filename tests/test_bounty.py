...
def test_wave_parser_anchors_target_and_repairs_clipped_zero():
...
def test_wave_parser_allows_bare_clear_number_only_for_card_crop():
    assert bounty._extract_wave_targets(["Clear 30"]) == []
    assert bounty._extract_wave_targets(
...
def test_detect_objectives_uses_card_local_wave_consensus(monkeypatch):
    ...
    reads = iter([
        "0/1", "0/1",  # progress strip
        "Clear Wave 20 of", "Clear Wave 20 of",  # unnormalized crop
        "Clear Wave 20 of", "Clear Wave 20 of",  # normalized raw crop
        "Clear 30", "Clear 30",  # normalized contrast crop
    ])
